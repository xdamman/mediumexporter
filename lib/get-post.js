...
/**
 * @typedef {import('..').GetPostOptionsReturnObject | import('..').GetPostOptionsReturnString} Options
 ...
 * @return {Promise<Story | string | undefined>}
 */ async function(mediumURL, params = {}) {
  /** @type {Options} */
  const options = params
  const { format = 'md', imagesPath = 'images' } = options

  if (
    !(
      mediumURL?.substr(0, 18) == 'https://medium.com' ||
      mediumURL?.match(/https:\/\/.*.medium.com/)
    )
  ) {
  ...
  if (s.virtuals.previewImage) {
    ...
    const { imageId: id, originalWidth } = s.virtuals.previewImage
    images.push(
      utils.getImageSrc({
        id,
        originalWidth
  ...
  return Promise.all(promises)
    .then(async results => {
      if (!!images.length) {
        ...
        let outputPath = path.join(output, story.slug, imagesPath)
        if (!!options.jekyll) {
      ...
      if (options.frontmatter) {
        ...
        if (story.featuredImage) {
          outputText += `image: ${story.images[0]}\n`
        }
      ...
      let outputPath = `${output}/${story.slug}.${format}`

      if (!!options.jekyll) {
        outputPath = `${output}/${story.date.slice(0, 10)}-${
          story.slug
        }.${format}`
      }
      if (output) {
        if (!!images.length && !options.jekyll) {
          outputPath = path.join(output, story.slug) + `/index.${format}`
        }
