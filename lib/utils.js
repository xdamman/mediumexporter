...
async function downloadImages(images, options = {}) {
  ...
  for (const image of images) {
    ...
    let { root, dir, base, ext } = parse(join(options.imageFolder, file))
    if (!ext) {
      base += '.jpg'
    }
    fs.writeFileSync(format({ root, dir, base }), response, 'base64')
    articleImages.push(base)
  }
...
function getImageSrc({ id, originalWidth }) {
  const imgwidth = parseInt(originalWidth, 10)
  return `${MEDIUM_IMG_CDN + Math.max(imgwidth * 2, 2000)}/${id}`
}

function processSection(s, slug, images, options = {}) {
  let section = ''
  if (s.backgroundImage) {
    const imgsrc = getImageSrc(s.backgroundImage)
    images.push(imgsrc)
...
async function getGitHubEmbed(iframesrc, options = {}) {
  ...
  if (json.payload.value.gist) {
    ...
    return mdSoureCode
  } else if (json.payload.value.iframeSrc) {
    const { iframeSrc, iframeHeight } = json.payload.value
    return `<iframe src="${iframeSrc}" width="100%" height="${iframeHeight}" />`
  }
  console.log(
    `couldn't handle media payload:\n${JSON.stringify(
      json.payload.value,
      null,
      2
    )}`
